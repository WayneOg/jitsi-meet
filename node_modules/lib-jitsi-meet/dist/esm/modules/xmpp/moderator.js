...
export default class Moderator extends Listenable {
    ...
    sendConferenceRequest(roomJid) {
        ...
        return new Promise((resolve, reject) => {
            if (this.mode === 'xmpp') {
                ...
                this.connection.sendIQ(this._createConferenceIq(roomJid), result => this._handleIqSuccess(roomJid, result, resolve, reject), error => this._handleIqError(roomJid, error, resolve, reject));
    ...
    _handleSuccess(roomJid, conferenceRequest, callback, errorCallback) {
        ...
        if (conferenceRequest.ready) {
            ...
            window.setTimeout(() => this.sendConferenceRequest(roomJid)
                .then(callback).catch(errorCallback), waitMs);
        }
    }
    /**
     ...
     * @private
     */
    _handleError(roomJid, sessionError, notAuthorized, callback, errorCallback) {
    ...
    _handleIqError(roomJid, error, callback, errorCallback) {
        ...
        const invalidSession = Boolean(exists(error, ':scope>error>session-invalid') || exists(error, ':scope>error>not-acceptable'));
        // Not authorized to create new room
